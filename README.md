## Descripción
- App sobre establecimientos de salud publicos y provados, donde muestra informacion sobre aquellos
- Este proyecto fue desarrollado utilizando el paradigma de Programación Orientada a Objetos (POO) para asegurar un código mantenible y escalable.
- fue implementado en el 2021. Ultima actualizacion : diciembre del 2021.
- Primera version de esta App - sin comentarios- VERSION LIMPIA, que se subio a la play store en el 2021.
- geolocalizacion de establecimeintos 

## Tecnologías
- java
- SQLITE nativo
- API GOOGLE MAPS
- API RUTAS
- Volley: para consultas a la web serviceg
- XML

## Arquitectura
patron de arquitectura n-Capas


## Autor
ELMER ANTONIO RODRIGUEZ
